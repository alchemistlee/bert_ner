...
model_dir = '/home/zhangyong/code/nlp/BERT-BiLSTM-CRF-NER/output_20190615/'
bert_dir = '/home/zhangyong/code/nlp/chinese_L-12_H-768_A-12'
...
def predict_online():
    ...
    with graph.as_default():
        ...
        while True:
            ...
            print('your input is:{0}, size = {1}'.format(sentence, len(sentence)))
            ...
            print('label size = {0}'.format(len(pred_label_result[0])))
            #todo: 组合策略
...
def convert_id_to_label(pred_ids_result, idx2label):
    ...
    for row in range(batch_size):
        ...
        print(res)
        print('array size = {}'.format(len(res))) 
...
def strage_combined_link_org_loc(tokens, tags):
    ...
    entity, date, number, metric  = eval.get_result(tokens, tags)
    print_output(entity, 'ENTITY')
    print_output(date, 'DATE')
    print_output(number, 'NUMBER')
    print_output(metric, 'METRIC')
...
class Result(object):
    ...
    def get_result(self, tokens, tags, config=None):
        ...
        return self.entity, self.date, self.number, self.metric
    ...
    def append(self, word, start, end, tag):
        if tag == 'METRIC':
            self.metric.append(Pair(word, start, end, 'METRIC'))
        elif tag == 'NUMBER':
            self.number.append(Pair(word, start, end, 'NUMBER'))
        elif tag == 'ENTITY':
            self.entity.append(Pair(word, start, end, 'ENTITY'))
        elif tag == 'DATE':
            self.date.append(Pair(word, start, end, 'DATE'))
        else:
