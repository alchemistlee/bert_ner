...
def eval(model, iterator, f, alpha):
    ...
    for rid, (words, is_heads, tags, y_hat_1, y_hat_2, x) in enumerate(zip(Words, Is_heads, Tags, Y_hat_1, Y_hat_2, X)):
        ...
        if preds_1 == preds_2:
            ...
            candidates = violent_combine(y_hat_1, y_hat_2)
            #__import__('ipdb').set_trace()
            starttime = datetime.datetime.now()
            for cand in candidates:
                likelihood_1 = get_likelihood(model, x, cand, 1)
                likelihood_2 = get_likelihood(model, x, cand, 2)
                ensemble_likelihood = alpha * likelihood_1 + (1 - alpha) * likelihood_2
                if ensemble_likelihood > max_likelihood:
                    #print('likelihood_1: {0}, likelihood_2: {1}, ensemble_likelihood: {2}'.format(likelihood_1, likelihood_2, ensemble_likelihood))
                    max_likelihood = ensemble_likelihood
                    best_preds = [idx2tag[hat] for hat in cand]
            endtime = datetime.datetime.now()
            print('parsing, candidates size = [{0}, cost [{1}]s]'.format(len(candidates), (endtime-starttime).seconds))
        
            # count
#            if best_preds == preds_1:
#                print('crf1 hit....')
#            if best_preds == preds_2:
#                print('crf2 hit....')
...
if __name__ == "__main__":
    ...
    parser.add_argument("--logdir", type=str, default="checkpoints/ensemble")
