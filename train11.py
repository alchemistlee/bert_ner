...
def eval(model, iterator, f):
    ...
    with torch.no_grad():
        for i, batch in enumerate(iterator):
            ...
            def get_mask(x, y_hat):
                # TODO 两种方案，1，单个sample遍历 2.直接给入一个mask
                y_mask = torch.zeros_like(x, dtype=torch.uint8)
                _tmp = torch.ones_Like(y, dtype=torch.unit8)
                for i in range(_tmp.shape[0]):
                    mask[i,:len(_tmp[i])].copy_(_tmp[i])
                return mask.ne(0)

            with torch.no_grad():
                for i in range(x.shape[0]):
                    #model.compute_loss(x[0][:24].unsqueeze(0), torch.tensor(y_hat[0]).unsqueeze(0))
                    logprob = model.compute_loss(x[i, :len(y_hat[i])].unsqueeze(0), torch.tensor(y_hat[i]).unsqueeze(0), 'none')
                    if logprob.item() < 0:
