...
def test():
    ...
    res = eval(tokenizer, model, eval_iter)
    if not res:
    	return json.dumps({}, ensure_ascii=False)
    return json.dumps(res, ensure_ascii=False)

def eval(tokenizer, model, iterator):
    model.eval()
    ...
    for rid, (words, is_heads, tags, y_hat_1, y_hat_2, x) in enumerate(zip(Words, Is_heads, Tags, Y_hat_1, Y_hat_2, X)):
        ...
        res.append(eval.result_to_json(words.strip().split()[1:-1], best_preds[1:-1]))
